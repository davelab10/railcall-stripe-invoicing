# Testing report — `dave/stripe-invoicing` v1.4.0
...
| Check | Result |
|---|---|
| Module identity | `dave/stripe-invoicing` v1.4.0 |
| Command registration | 32/32 resolved |
| Classification | 19 `read`; 13 `write_requires_approval` |
| Module regression | 31 passed; 34 subtests |
...
The final result was **31 passed with 34 subtests**. All 32 commands resolve, and the 13 mutation commands remain `write_requires_approval` with `receipt_required: true`.
...
All 32 action IDs are unique. The module does not request subprocess or filesystem-write access. Receipt and egress checks did not find secret-pattern leakage in audited outputs.
...
Module v1.4.0 is the verified contest state: 32 resolved commands, 13 approval-controlled writes, incremental/schedulable invoice history, privacy-minimized structured AI, bounded retry behavior, vault-local credentials, approval-derived idempotency, signed evidence, and no source changes required for the final verification.
